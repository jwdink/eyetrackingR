...
make_time_sequence_data <- function (data,
  ...
  df_tb_sum = summarise(df_grouped, N = n())
  df_tb_sum = summarise(df_tb_sum, N = mean(N))
  # if last time bin too small (<1/3 expected size), collapse it into the previous time bin
  too_small_tb = df_tb_sum$N[which.max(df_tb_sum$TimeBin)] < time_bin_size / 3
  if (too_small_tb) data$TimeBin[data$TimeBin==max(df_tb_sum$TimeBin)] <- max(df_tb_sum$TimeBin) - 1
                              ...
                              ...)
{
  ...
  if (!require("lme4")) {
    if (test == "lmer") stop("Please install the 'lme4' package.")
  }
...
#'
#' @return list A ggplot list object
plot.time_window_data <- function(data, data_options, predictor_columns, dv = "Prop") {
