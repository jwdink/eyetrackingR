...
look_at_aoi <- function(ppt_offset, item_offset, orig_dat, time_in_trial) {
  ...
  prob_vec <- boot::inv.logit(baseline_odds + ppt_offset + item_offset + time_offset)
  #cat("\n", length(prob_vec))
...
Boot-splines can't control the false-alarm rate when items potentially vary a lot. Random variation in the items creates what looks like systematic patterns in the looking-times when you ignore those items.
...
```{r, warning=FALSE}
cluster_anal_md <- analyze_time_clusters(df_clust_md, within_subj = T, paired=T, samples = 200)
...
Boot-splines doesn't even flinch. 
