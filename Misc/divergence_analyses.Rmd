...
results1 <- bind_rows( pbreplicate(50, simplify = FALSE, expr = {
  ...
  with(tb_anal, 
       data.frame(false_alarm_bins = sum(Sig[Time <  effect_start-tb_size]),
            ...
            true_effect_onset = effect_start + mean(df$RT, na.rm=TRUE)
       ))
}) )
...
cat(
  ...
  "(",round(quantile(boot::boot(results1$first_discovery_time, f, R=500)$t, probs = c(.025, .975))),")",
...
cat(
  ...
  "(",round(quantile(boot::boot(results2$first_discovery_time, f, R=500)$t, probs = c(.025, .975))),")",
...
cat(
  ...
  "(",round(quantile(boot::boot(results3$first_discovery_time, f, R=500)$t, probs = c(.025, .975))),")",
...
cat(
  ...
  "(",round(quantile(boot::boot(results4$first_discovery_time, f, R=500)$t, probs = c(.025, .975))),")",
