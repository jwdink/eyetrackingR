...
if (boot_clusters == TRUE) {
  time_cluster_low_thresh_boot_df <- params_to_test_template_limited
  time_cluster_low_thresh_boot_df$ThresholdAlpha <- .05
...
```{r}
### Stil messing around with this...

# ggplot(data = filter(summary_per_cell_dr_brock, Measure != "FalseDiscoveryRate"), 
#        mapping = aes(x= Diff, y = Mean, fill = TestType)) %>%
#   make_graph() + ylab("Proportion") + xlab("Difference in Preference Across Conditions") +
#   facet_grid(Measure ~ NumSubjectsText+Test+Adjustment, scales = "free_x")
# 
# graph_FA <- filter(summary_per_cell_fa, 
#               Measure != "FalseDiscoveryRate",
#               Test != "gca") 
# ggplot(data = graph_FA, mapping = aes(x= NumSubjectsText, y = Mean, fill = TestType)) %>%
#   make_graph() + ylab("Proportion") +
#   facet_grid(Measure ~ .) + geom_hline(yintercept = .05, linetype="dashed")
```
...
summary_overall_wide <- master_df %>%
                    ...
                    summarise(ErrorRate    = mean(FABins/num_no_effect_time_bins),
                              ...
                              FalseDiscoveryRate = mean( (FABins/num_no_effect_time_bins) / 
                                                                   ((CDBins/num_effect_time_bins) + (FABins/num_no_effect_time_bins)),na.rm=T)
                    )
