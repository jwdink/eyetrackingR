...
tb_size <- 50
sim_factor <- 20 # of simulations for each Nsubjects/Nitems/pref cell

  # create vectors for permutations
  N_subjects = rep(c(10,30,50,70,90), each=sim_factor*3) # multipying factor equals length(N_items) * pref
  N_items = c(10)
  pref = rep(c(.65,.75,.85),each=1) # multipying factor equals length(N_items)
...
t_tests_bonf_results <- data.frame(matrix(unlist(pbapply(t_tests_bonf, 1, function (x) {
  N_subjects = as.numeric(x[['N_subjects']])
  N_items = as.numeric(x[['N_items']])
  pref = as.numeric(x[['pref']])
...
t_tests_fdr_results <- data.frame(matrix(unlist(apply(t_tests_fdr, 1, function (x) {
  N_subjects = as.numeric(x[['N_subjects']])
  N_items = as.numeric(x[['N_items']])
  pref = as.numeric(x[['pref']])
...
clusters_fdr_results <- data.frame(matrix(unlist(apply(clusters_fdr, 1, function (x) {
  ...
  if ( nrow(get_time_clusters(cl_dat))>0 ) {
    ...
    cl_clusts <- get_time_clusters(cl_anal)
    cl_clusts$NumBins <- with(cl_clusts, (EndTime - StartTime)/tb_size)
    cl_sig_clusts <- filter(cl_clusts, cl_clusts$Probability<.05/fdr_correction)
    out <- with(cl_sig_clusts, 
              list(
                avg_rt = mean(df$RT, na.rm=TRUE),
                rt_lower_quart = quantile(df$RT, probs = .25),
...
ggplot(subset(summary_long, Measure != 'NumClusters' & N_subjects == 90), aes(x=as.factor(N_subjects), y=Value, fill=test)) +
                  scale_fill_manual(values=c('#333333','#444444','#555555',
                                             '#4f186d','#b864e7','#d2b6ea',
                                             '#fa8072','#ee8262')) + #,'#ff7256')) +
