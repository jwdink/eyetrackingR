...
boot_clusters <- TRUE # set to TRUE to run bootspline clusters
...
if (!file.exists('sim_results/boot_splines_results.RDS')) {
  ...
  saveRDS(boot_splines_results, file = "sim_results/boot_splines_results.RDS", compress="xz")
}
```
...
if (!file.exists('sim_results/boot_splines_bonf_results.RDS')) {
  ...
  saveRDS(boot_splines_bonf_results, file = "sim_results/boot_splines_bonf_results.RDS", compress="xz")
}
```
...
for (i in seq_along(time_cluster_low_thresh_results)) {
  ...
  time_cluster_low_thresh_df[i,"FirstDiscTime"] <- with(model, ifelse(sum(!is.na(ClusterStartTimeWithinEffect))>0, yes = min(ClusterStartTimeWithinEffect,na.rm=TRUE), no = NA))
  }
  else {
...
for (i in seq_along(time_cluster_high_thresh_results)) {
  ...
  if (nrow(model) > 0) {
    ...
    time_cluster_high_thresh_df[i,"FirstDiscTime"] <- with(model, ifelse(sum(!is.na(ClusterStartTimeWithinEffect))>0, yes = min(ClusterStartTimeWithinEffect,na.rm=TRUE), no = NA))
  }
  else {
...
if (boot_clusters == TRUE) {
  if (!file.exists('sim_results/time_cluster_low_thresh_boot_results.RDS')) {
    message("===============================================================================")
    message("Warning: Didn't find boot-cluster RDS. Running full simulation. Will take days.")
    message("===============================================================================")
    time_cluster_low_thresh_boot_results <- foreach(i=1:nrow(time_cluster_low_thresh_boot_df)) %dopar% {
      ...
      cl_dat  <- make_time_cluster_data(df_time, predictor_column = "Condition", test = "boot_splines", within_subj = FALSE, alpha = time_cluster_low_thresh_boot_df[i,"ThresholdAlpha"])
...
if (boot_clusters == TRUE) {
  ...
  for (i in seq_along(time_cluster_low_thresh_boot_results)) {
    element <- time_cluster_low_thresh_boot_results[[i]]
    if (is.list(element)) { # <--- missing one sim
      if (!all(names(element)=="ROW")) {
        model <- get_time_clusters(element)
        model <- model %>%
          mutate(Sig = Probability < .05,
                 PreEffectTime = ifelse(effect_start-StartTime > 0, effect_start-StartTime, 0),
                 ClusterStartTimeWithinEffect = ifelse(effect_start <= StartTime, StartTime, effect_start),
        ...
        time_cluster_low_thresh_boot_df[i,"FirstDiscTime"] <- with(model, 
                                                                   ifelse(sum(!is.na(ClusterStartTimeWithinEffect))>0, 
      ...
      #if(!all(time_cluster_low_thresh_boot_df[i,1:8] == element$ROW)) warning("Rows don't match!", i)
    }
  }
...
if (boot_clusters == TRUE) {
  ...
  for (i in seq_along(time_cluster_high_thresh_boot_results)) {
    element <- time_cluster_high_thresh_boot_results[[i]]
    if (is.list(element)) { # <--- missing one sim
      if (!all(names(element)=="ROW")) {
        model <- get_time_clusters(element)
        model <- model %>%
        ...
        time_cluster_high_thresh_boot_df[i,"FirstDiscTime"] <- with(model, 
                                                                   ifelse(sum(!is.na(ClusterStartTimeWithinEffect))>0, 
                                                                          yes = min(ClusterStartTimeWithinEffect,na.rm=TRUE), 
                                                                          no = NA))
      } else {
...
master_df$TestType <- with(master_df, 
                           factor(x = ifelse(Adjustment != '',paste0(Test, ' (', Adjustment, ')'),stringr::str_trim(Test)), 
                                  levels = c('t_test', 't_test (bonferroni)', 
                                             ...
                                             "time_cluster_boot (low_threshold)", "time_cluster_boot (high_threshold)")
                           ))
...
png(filename = "plot-discoveries.png",
    width = 4000, height = 2700, units = "px", pointsize = 16,
    bg = "white", res = 400)
p
dev.off()
p
...
## Pub-Quality Option 2:

```{r}
### Still messing around with this...

summary_key <- master_df %>%
  group_by(NumItems, Alpha, Test, Adjustment, TestType, NumSubjects) %>%
  summarise(FWER         = get_mean_ci(FABins != 0),
            DiscoveryRate = get_mean_ci(CDBins/num_effect_time_bins)
  ) %>%
  tidyr::gather(key = "Measure", value = "Value", FWER:DiscoveryRate) %>%
  tidyr::separate(col = Value, sep = ", ", into = c("Mean", "CI_Low", "CI_High"), remove = TRUE) %>%
  mutate(Mean = as.numeric(Mean), CI_Low = as.numeric(CI_Low), CI_High = as.numeric(CI_High),
         DashedLine = ifelse(Measure %in% c('ErrorRate','FWER','FalseDiscoveryRate'), .05, .80))

ggplot(summary_key, aes(x = Test, y = Mean, fill = TestType)) %>%
  make_graph() + scale_y_continuous(name = "Proportion") +
  geom_hline(aes(yintercept = DashedLine), linetype="dashed") +
  facet_grid(NumSubjects ~ Measure)
```
...
summary_overall_wide <- master_df %>%
                    ...
                    summarise(ErrorRate    = mean(FABins/num_no_effect_time_bins),
                              ...
                              FalseDiscoveryRate = mean( (FABins/num_no_effect_time_bins) / 
                                                                   ((CDBins/num_effect_time_bins) + (FABins/num_no_effect_time_bins)),na.rm=TRUE) )
