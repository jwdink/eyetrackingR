...
if (!file.exists('sim_results/gca_results.RDS')) {
  gca_results <- foreach(i=1:nrow(gca_df)) %dopar% {
    ...
    df_time <- make_time_sequence_data(data = df, time_bin_size = tb_size, aois = "AOI1", predictor_columns = "ConditionC",
                                       summarize_by = "Participant")
    
    # whole window:
    gca_anal_whole_window <- lmer(Prop ~ (ot1+ot2+ot3)*ConditionC + (1+ot1+ot2+ot3|Participant), data = df_time)
    ...
    df_time_no_effect_window <- df %>%
      ...
      make_time_sequence_data(time_bin_size = tb_size, aois = "AOI1", predictor_columns = "ConditionC",
                                       summarize_by = "Participant")
    gca_anal_no_effect_window <- lmer(Prop ~ (ot1+ot2+ot3)*ConditionC + (1+ot1+ot2+ot3|Participant), 
...
master_df$TestType <- with(master_df, 
                           factor(x = paste(Test, Adjustment), 
...
ggplot(filter(summary_per_cell_dr, Measure=="DiscoverRate"), aes(x=Test, y=Mean, fill=TestType)) %>%
  make_graph() + scale_y_continuous(name='DiscoverRate') + facet_grid(Pref ~ NumSubjects)
```
...
summary_overall <- master_df %>%
  ...
  summarise(ErrorRate    = get_mean_ci(FABins/num_no_effect_time_bins),
            ...
            FWDR         = get_mean_ci(CDBins != 0),
            FalseDiscoverRate = get_mean_ci( (FABins/num_no_effect_time_bins) / 
                                               ((CDBins/num_effect_time_bins) + (FABins/num_no_effect_time_bins)))
  ) %>%
  tidyr::gather(key = "Measure", value = "Value", ErrorRate:FalseDiscoverRate) %>%
  tidyr::separate(col = Value, sep = ", ", into = c("Mean", "CI_Low", "CI_High"), remove = TRUE) %>%
  mutate(Mean = as.numeric(Mean), CI_Low = as.numeric(CI_Low), CI_High = as.numeric(CI_High))

summary_overall <- filter(summary_overall, !Test %in% c("time_cluster_boot", "gca") ) # remove when we have data here

ggplot(summary_overall, aes(x = Test, y = Mean, fill = TestType)) %>%
  make_graph() + scale_y_continuous(name = "Proportion") + facet_grid(Measure ~ .)
