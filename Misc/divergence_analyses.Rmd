...
summary <- all_tests %>%
           ...
           summarise(
             DiscoveryRate = mean(correct_discovery_bins) / (num_time_bins/2),
             FamilyWiseDiscoveryRate = (1-mean(correct_discovery_bins == 0)),
             MeanFirstDivergence = mean(first_discovery_time, na.rm=TRUE),
             ErrorRate = mean(false_alarm_bins) / (num_time_bins/2),
...
summary_long <- summary %>%
           tidyr::gather('Measure', 'Value', DiscoveryRate, FamilyWiseDiscoveryRate, ErrorRate, FamilyWiseErrorRate,NumClusters)
