...
describe_data <- function(data, data_options, describe_column, group_columns) {
  
  df_grouped <- group_by_(data, .dots = as.list(group_columns))
  df_summarised <- summarise_(df_grouped, 
                             .dots = list(Mean = interp( ~mean(DV_COL, na.rm=TRUE),     DV_COL = as.name(describe_column) ),
