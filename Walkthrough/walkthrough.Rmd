...
response_window_agg <- make_window_data(response_window, data_options, aoi='Animate', condition_columns=c('Target','Age','MCDI_Total'))
...
```{r}
bootstrapped_familiar <- make_bootstrapped_data(response_time, data_options, condition_column = 'Target', within_subj = TRUE, samples = 1000, resolution = 100, alpha = .05, smoother = 'smooth.spline') 
...
response_time_by_subj$ParticipantName = paste(response_time_by_subj$ParticipantName, response_time_by_subj$Target, sep="_")

df_timeclust = make_time_cluster_data(response_time_by_subj, data_options, condition_column = "Target", 
                                 within_subj=FALSE, paired = FALSE, threshold = 2)
plot(df_timeclust) + theme_bw()

# need a summary method for time_cluster_data

foo = analyze_time_clusters(df_timeclust, data_options, samples = 100, within_subj = FALSE)
hist(foo$null_distribution)
abline(v= attr(foo, "eyetrackingR")$cluster_sum_stat)
attr(foo, "eyetrackingR")$cluster_sum_stat

# need a summary and plot method for time_cluster_analysis

# 
# 
# hist(bar$null_distribution, breaks = 25)
# abline(v = bar$cluster_sum_stats)
# 
# sapply(bar$cluster_sum_stats, FUN = function(x) mean(x<bar$null_distribution))
