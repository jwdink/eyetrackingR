...
response_window$Target <- ifelse(grepl('(Spoon|Cow|Bottle)', response_window$Trial), 'Inanimate', 'Animate')
...
response_window_agg <- window_shape(response_window, data_options, aoi='Animate', c('Target','Age','MCDI_Total'))

plot(response_window_agg, data_options, x_axis_column="Age")
...
model <- lmer(elog ~ TargetC + (1 + TargetC | Trial) + (1 + TargetC | ParticipantName), data = response_window_agg, REML = F)
summary(model)
...
model <- lmer(elog ~ TargetC*(ot1 + ot2 + ot3 + ot4 + ot5) + (1 + TargetC | Trial) + (1 | ParticipantName), data = response_time, REML = F)
summary(model)
drop1(model,~.,test="Chi")
