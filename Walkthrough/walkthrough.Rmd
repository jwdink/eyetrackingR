...
response_window_agg <- make_time_window_data(response_window, data_options, aoi='Animate', predictor_columns=c('Target','Age','MCDI_Total'))
...
t.test(ArcSin ~ Target, data=subject_means, paired=TRUE)
```
...
onset_switches <- make_switch_data(onsets, data_options, predictor_columns = "Target")

plot(onset_switches, data_options, predictor_columns = "Target") 
...
```{r}
response_time_by_subj <- make_time_sequence_data(response_window, data_options, time_bin_size = 100, 
                                 predictor_columns = c("Target"),
                                 aoi = c("Animate"), 
                                 summarize_by = "ParticipantName"
                            )

df_timeclust = make_time_cluster_data(response_time_by_subj, data_options, 
                                      test= "t.test", paired = TRUE,
                                      predictor_column = "Target", 
                                      threshold = 2)
plot(df_timeclust) 

clust_analysis = analyze_time_clusters(df_timeclust, data_options, within_subj = TRUE, paired=TRUE)
clust_analysis
