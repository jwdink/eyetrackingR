...
Our first approach is to look for runs of significant differences between our conditions after smoothin the data in a series of time bins (similar to Wendt et al., 2014). This involves:

1. Fitting a smoothing curve to the data (using either `smooth.spline()`, `loess()`, or a moving average)
...
This is a useful technique for estimating the timepoints of divergence between two conditions, while the smoothing helps remove minor deviations that might disrupt what would otherwise be considered a single divergent period. This can be especially helpful in infant data, which can be extremely noisy. Note that this approach does not explicitly control for Type-I error rates, and it can only deal with two-level factors.
...
To begin, we need to use `make_time_sequence_data` to generate a time-binned dataframe. Here we'll explicitly summarize_by ParticipantName but, regardless of whether you do this or leave it as trial-by-trial data, `make_boot_splines_data` (below) will summarize by ParticipantName.
...
2. Take the time-bins whose test passed the necessary statistic (e.g., t > 2.26), and group them by adjacency. We will call these time-clusters.
...
To perform this procedure using eyetrackingR, we'll first create a time sequence dataset using `make_time_sequence_data` (as we did above):
...
```{r, warning=FALSE}
# only bootstrap for p-values if we actually have clusters identified
