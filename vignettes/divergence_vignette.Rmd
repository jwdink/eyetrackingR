...
To begin, we need to use `make_time_sequence_data` to generate a time-binned dataframe. Here we'll explicitly `summarize_by` ParticipantName Note that, regardless of whether you do this or leave it as trial-by-trial data, `make_boot_splines_data` (below) will automatically summarize by participant as a first step.
