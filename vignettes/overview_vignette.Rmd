...
# visualize GCA model
plot(response_time, predictor_column = "Target", dv = "Elog", model = model) +
  theme_light()
...
onset_switches <- make_switch_data(onsets, predictor_columns = c("Target","MCDI_Total"))
...
onset_switches$FirstAOIC <- as.numeric( scale(onset_switches$FirstAOIC, center=TRUE, scale=FALSE) ) # use as.numeric with scale or dplyr rebels
onset_switches$TargetC <- ifelse(onset_switches$Target == 'Animate', .5, -.5)
onset_switches$TargetC <- as.numeric( scale(onset_switches$TargetC, center=TRUE, scale=FALSE) )
