...
response_time <- make_time_sequence_data(response_window, data_options, time_bin_size = 250, 
...
response_window$TrialTarget <- 
  ifelse(response_window$Target == 'Animate', response_window$Animate, response_window$Inanimate)
response_window$TrialDistractor <- 
  ifelse(response_window$Target == 'Animate', response_window$Inanimate, response_window$Animate)
...
plot(onset_switches, predictor_columns = "Target") + theme(legend.position="none")
...
clust_analysis = analyze_time_clusters(df_timeclust, within_subj = TRUE, paired=TRUE, samples=100)
clust_analysis
