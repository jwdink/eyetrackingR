...
Here, we'll visualize the time-course of looking. To do this, we use eyetrackingR's `time_sequence_data` type. This bins our data into time-bins of our choosing. Here, we use 100ms time-bins. We use the 'arcsin square root' transformation on proportion-looking.
...
The simplest test we can do aggregates across the entire time-window, and compares the proportion-looking across conditions using a simple t-test on participants' means. 
