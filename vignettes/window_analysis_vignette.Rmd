...
You might notice that these are higher than we might have expected, based on the raw proportions. This could suggest our choice of transformation (empirical logit) was problematic. It's important to choose a transformation that satisfies the assumptions of the model we are using. We can plot the fitted vs. residuals of our model using the `plot` method for `lmer`:
