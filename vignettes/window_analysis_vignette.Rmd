...
While the first DV is the most intuitive, you might consider using the latter two DV's to account for the bounded nature of proportions. The boundedness of proportions is an issue because it can mean that CI's around your estimates fall above 1 or below 0 (which is not possible) and that there is an inherent link between group means and variance (means closer to 0 and 1 will necessarily have lower variance than means around .5). Here we will use the ArcSin DV.
...
This linear model shows no main effects or interactions of Target with those predictors, but confirms the huge effect of Target.
...
Because we sum-coded our `TargetC` predictor before fitting the model, we can calculate the looking to the Animate in the Animate target condition by adding .5*2.48 (the TargetC estimate) to 1.15 (the Intercept estimate):

```{r, warning=FALSE}
condition_estimate <- 1.15 + .5*2.48
```
...
summary(model_time_window_add_predictors)

# cleanly show p-values
