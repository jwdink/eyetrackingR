...
We will also perform a few sanity checks (as always). For more information on these, look at the [window analysis vignette](window_analysis_vignette.html).

```{r, warning=FALSE}
# plot each participant's condition means
...
ggplot(data_summary, aes(x=Target, y=Mean, group=ParticipantName)) +
...
To do so, we can use eyetrackingR's `time_sequence_data` type to create a dataset and its `plot` function to visualize it. This function requires us to bin our data so that, within each bin, we calculate a proportion of looking to our Animate AOI. Here we use 100ms time-bins. 
...
Growth curve analysis (GCA) lets us model the timecourse of attention by fitting curves to proportion-looking over the course of the trial, and statistically assessing the bends in these curves.  Our implementation of a growth curve analysis modelled after Mirman (2008).
...
First, it includes the same dependent variable columns as `make_time_window_data`, giving us an option of analyzing raw proportions or two (likely more appropriate transformations):

* `Prop` -- the mean of raw proportion scores
* `Elog` -- the empirical logit for each of these proportions
* `ArcSin` -- the arcsine-root transformation for each of these proportions

Second, it gives us a series columns corresponding to 'orthogonal polynomial timecodes.' These are *uncorrelated* vectors that are matched to your trial lengths, each of which fit a growth function of a different form (e.g., linear, quadratic, cubic, quartic, ...).

We can visualize each of these vectors to give you an idea, as well as demonstrate that they are uncorrelated:
...
You can see the linear time code moving from the bottom left to top right corner of that plot (in black). Each of the other coloured lines corresponds to a different growth function. The idea behind GCA is that we can simulanteously regress differences between our conditions (here, Target) on each of these vectors to see which (in combination, or independently) best captures the pattern of growth in our data. Importantly, because they are uncorrelated, they are sure to capture *distinct* variance in our data.

Let's fit our first GCA model, including *only* the linear time code (i.e., saying that we want to ignore non-linear change over time):
...
This model reveals two main effects of TargetC and ot1, respectively. These may seem OK at first, but they are actually not a great match to our data (as revealed by the plot of their predictions). The main effect of TargetC says that, on average throughout each trial, participants looked more to the Animate target when it was named than when the Inanimate was named. This is accurate. The main effect of ot1 says that, on average, participants looked away from the Animate over the course of each trial. This is also accurate. What we are missing is a result that reflects that these differences between Target *emerged* after the start of the window.

To remedy this issue, we can allow for non-linear change overtime by including a few higher-order orthogonal polynomials:

```{r, warning=FALSE}
model_time_sequence <- lmer(Elog ~ TargetC*(ot1 + ot2 + ot3 + ot4) + (1 | Trial) + (1 | ParticipantName), data = response_time, REML = FALSE)

# commented out because this many random slopes takes too long to fit for a vignette
# model_time_sequence <- lmer(Elog ~ TargetC*(ot1 + ot2 + ot3 + ot4) + (1 + ot1 + ot2 + ot3 + ot4 | Trial) + (1 + ot1 + ot2 + ot3 + ot4 | ParticipantName), data = response_time, REML = FALSE)
...
In the case of ascertainign reaction times, one way to answer this question is by using [onset-contingent analyses](onset_contingent_analysis_vignette.html).
