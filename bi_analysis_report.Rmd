...
df4 = subset_by_window(df4, data_options, window_start="StartTime", window_end="StopTime")

foo = remove_trackloss(df4, data_options, delete_rows = TRUE)

df4_ = convert_non_aoi_to_trackloss(data = df4, data_options)
# 
...
df_time = time_analysis(data = df5, data_options, time_bin_size = 500, 
                        condition_columns = "TimeInExp",
                        dv = c("IA_bust","IA_waist","IA_hips","IA_thighs") )
plot(df_time, data_options, condition_column = "TimeInExp")
df_time = left_join(df_time, df_measures, by = c("Participant", "AOI") )

seq_bins = analyze_time_bins(data = df_time, data_options, condition_column = "TimeInExp", within = TRUE)
plot(seq_bins)
```
...
```{r, message=FALSE}
# fit_main_diss = lmer(ArcSin ~ DissBPC+TimeInSecC + BodyPart + DissNoBPC + (1+BodyPart|Participant) + (1+BodyPart|Image), 
#                      REML = FALSE,
#                      data = df_binned)
...
```{r, message=FALSE}
# fit_diss_int_time = lmer(ArcSin ~ DissBPC*TimeInSecC + BodyPart + DissNoBPC + (1+BodyPart|Participant) + (1+BodyPart|Image), 
#          REML = FALSE,
#          data = df_binned)
# drop1(fit_diss_int_time, test = "Chisq")
# tidy(fit_diss_int_time, effects = "fixed")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# 
# plot(df_cleaner, data_options, dv = "InAOI", group_factor = "DissBP", time_bin_size = 1000, type = 'empirical')
# 
# plot(df_cleaner, data_options, dv = "InAOI", group_factor = "DissBP", time_bin_size = 500, type = 'smoothed')
